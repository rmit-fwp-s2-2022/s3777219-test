...
export function validate(event , nameFieldParam, emailFieldParam,passwordFieldParam){
...
for (let i = 1; i <= 2; i++) {
        ...
        if ( /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(fieldsArray[1].value))
        {
        ...
        $('#'+fieldsArray[1].id).next('p').remove();
            ...
            $('#'+fieldsArray[1].id).next('p').remove();
            $( "<p id ='errorMsg' style = 'color: red;'>Invalid Format</p>" ).insertAfter( fieldsArray[1] );
            return 0;
        }
        if(/(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[^A-Za-z0-9])(?=.{8,})/.test(fieldsArray[2].value))
        {
            fieldsArray[2].style.borderColor = '#ced4da'
            $('#'+fieldsArray[2].id).next('p').remove();   
        }
        else{
            fieldsArray[2].style.borderColor = 'red'
            $('#'+fieldsArray[2].id).next('p').remove();
            $( "<p id ='errorMsg' style = 'color: red;'>Try Strong Password</p>" ).insertAfter( fieldsArray[2] );
            return 0;
        }
